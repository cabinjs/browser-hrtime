...
const open_page = async (page, name) => {
  return await page.goto(`file:${path.join(__dirname, name + '.html')}`, { waitUntil: 'domcontentloaded' });
};
...
describe('browser', () => {
  ...
  describe('process.hrtime()', () => {
    ...
    test('global var hrtime() should be a function', async () => {
      const hrtimeType = await page.evaluate(() => {
